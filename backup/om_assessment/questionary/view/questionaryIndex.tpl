   ...
   <div class="container">
     ...
     <div class="panel panel-default">
         ...
         <h2 class="sub-header marginleft">Questionary Type</h2>
          ...
          <form class="form-inline" role="form" action="../questionary/addQuest.php" method="POST">
                ...
                <label class="marginleft">ID</label>
                 {foreach from=$maxIDQuest item=maxID } 
                 {$id=$maxID[quest_ID]+1}
                 <input name="idQuestionary" type="text"  class="form-control" id="id_questionary"  value="{$id}" readonly>
                 {/foreach}
                </div>
         
                <div class="form-group">
                <label  class="marginleft" >Questionary</label>
                <input name="questionayType"type="text" class="form-control" value="{$quest}" placeholder="">
                </div>

                <div class="form-group">
                 <label class="marginleft">Description</label><br>
                 <div class="col-sm-4">
                 <textarea name="description" class="form-control" rows="2" cols="30">{$desc}</textarea>
                 </div>
                </div>
       ...
       <div class="row clearfix">
        ...
        {section name=qty loop=$questionaryInfo}
         <tbody>
          <tr>
          <td>{$questionaryInfo[qty].quest_ID}</td>
          <td>{$questionaryInfo[qty].quest_type}</td>
          <td>{$questionaryInfo[qty].quest_desc}</td>
          <td> <a href='../questionary/displayQuestionary.php?id={$questionaryInfo[qty].quest_ID}'><input class="btn btn-default" type="submit" name="editQuestionary" value="edit"></a> </td>
          </tr>
         </tbody>
          {/section}
        </table>
